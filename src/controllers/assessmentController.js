...
const ATTACH_TYPES = ["lesson", "module", "training"];
...
async function attachmentExists(type, id) {
  const table = type === "lesson" ? "lessons" : type === "module" ? "modules" : "training_programs";
...
const getAssessment = async (req, res) => {
  try {
    ...
    if (!ATTACH_TYPES.includes(attached_to_type) || !attached_to_id) return res.status(400).json({ success: false, message: "attached_to_type ('lesson', 'module', or 'training') and attached_to_id are required" });
...
const createAssessment = async (req, res) => {
  try {
    const body = req.body;
    if (!ATTACH_TYPES.includes(body.attached_to_type) || !body.attached_to_id) return res.status(400).json({ success: false, message: "attached_to_type ('lesson', 'module', or 'training') and attached_to_id are required" });
    ...
    if (!(await attachmentExists(body.attached_to_type, body.attached_to_id))) return res.status(404).json({ success: false, message: `${body.attached_to_type === "lesson" ? "Lesson" : body.attached_to_type === "module" ? "Module" : "Training program"} not found` });
    const id = uuidv4();
